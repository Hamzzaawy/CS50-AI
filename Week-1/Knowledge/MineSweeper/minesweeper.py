...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for a in range(max(0, i-1), min(i+2, self.height)):
            for b in range(max(0, j-1), min(j+2, self.width)):
                if (a,b) != (i,j):
                    cellNeighbors.add((a,b))               
        
        self.knowledge.append(Sentence(cellNeighbors, count))

        # mark additional cells as safe or mines
        self.UpdateKnowledgBase()


        inferences = self.NewInference()
    ...
    def NewInference(self):
        ...
        self.knowledge = [k for k in self.knowledge if k not in removals]
        return inferences


    def printing(self):
        print("Known Mines So Far: ", self.mines)
        print("Knowlege Base: ")
        for sent in self.knowledge:
            print("\t", sent.cells, " = ", sent.count)
